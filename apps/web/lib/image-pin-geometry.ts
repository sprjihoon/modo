...
export function clientToImageRelative(
  clientX: number,
  clientY: number,
  containerOrigin: { left: number; top: number },
  contain: Rect,
): { x: number; y: number } | null {
  if (contain.width <= 0 || contain.height <= 0) return null;
  const x = (clientX - containerOrigin.left - contain.left) / contain.width;
  const y = (clientY - containerOrigin.top - contain.top) / contain.height;
  if (x < 0 || x > 1 || y < 0 || y > 1) return null;
  return { x: clamp01(x), y: clamp01(y) };
}

export function clientToImageRelativeClamped(
  clientX: number,
  clientY: number,
  containerOrigin: { left: number; top: number },
  contain: Rect,
...
export function containerRelativeToImageRelative(
  pin: { x: number; y: number },
  container: Size,
  contain: Rect,
): { x: number; y: number } {
  if (contain.width <= 0 || contain.height <= 0) {
    return { x: clamp01(pin.x), y: clamp01(pin.y) };
  }
  const x = (pin.x * container.width - contain.left) / contain.width;
  const y = (pin.y * container.height - contain.top) / contain.height;
  return { x: clamp01(x), y: clamp01(y) };
}
