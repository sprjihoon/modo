...
### 맥북에서 `1.0.3+31` (지금 이 빌드)

Windows에서는 IPA/AAB를 만들지 않는다. 맥북에서 `main`을 받은 뒤 아래만 실행한다. `pubspec.yaml`은 이미 `1.0.3+31`.

포함 내용: 3단계 담기 제거 · 4단계에서만 장바구니 담기(`items[]`) · 포인트 intent 숨김 · 가격안내 순서.
...
29를 덮어쓰지 말고 31을 새로 올린다. 스토어에 올린 뒤에만 어드민 **앱 버전**을 `1.0.3` / 31으로 바꾼다.
...
| 버전 | `pubspec.yaml` → `1.0.3+31` (다음 맥북 빌드) · 스토어에 올린 코드는 `+29` |
| 최근 UX | 4단계에서만 장바구니 담기 · 가격안내 순서 · 포인트 intent 숨김 |
| AAB | `build/app/outputs/bundle/release/app-release.aab` · 백업 `~/Documents/modo-android-signing/app-release-1.0.3+31.aab` |
...
| 최신 업로드 | **`1.0.3 (29)`** 판매 중. 다음 맥북 빌드는 **31** (장바구니 담기 + 가격안내 순서) |
