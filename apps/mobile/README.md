...
| 현재 트랙 | 비공개 테스트(Alpha) — **`20 (1.0.1)` 검토 전송** (2026-08-15) |
| 버전 | `pubspec.yaml` → `1.0.1+20` |
| 최근 UX | 비회원 홈·가격표 · 권한은 기능 사용 시 · 실제 비밀번호 변경 · Apple 웹 세션 |
| AAB | `build/app/outputs/bundle/release/app-release.aab` · 백업 `~/Documents/modo-android-signing/app-release-1.0.1+20.aab` |
...
| 최신 업로드 | **`1.0.1 (20)`** (게스트 둘러보기 · iPad Apple 웹 세션) |
| App Store 심사 | 빌드 16 **REJECTED** → **20** `WAITING_FOR_REVIEW` |
...
flutter build ipa --release --build-name=1.0.1 --build-number=20 \
