...
스토어 빌드(IPA/AAB)는 **하루에 한 번만** 한다. 웹은 기다리지 않고 `modo.io.kr`에 바로 적용한다. 앱은 지금은 빌드하지 않고, 오늘(2026-09-01) **몇 시간 뒤에** 하루 1회 예정.
...
# Play AAB
flutter build appbundle --release --build-name=1.0.6 --build-number=42
# → build/app/outputs/bundle/release/app-release.aab
# 백업: ~/Documents/modo-android-signing/app-release-1.0.6+42.aab

# App Store / TestFlight IPA
flutter build ipa --release --build-name=1.0.6 --build-number=42 \
...
| 최신 업로드 | **`1.0.5` 판매 중**. **`1.0.6 (42)`** 심사 대기 |
