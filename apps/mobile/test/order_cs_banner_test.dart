...
void main() {
  test('1회차이고 CS 없으면 배너 숨김', () {
    expect(showCsBanner(cycle: 1, csStatus: null), false);
  });

  test('재작업 회차는 배너 표시', () {
    expect(showCsBanner(cycle: 2, csStatus: 'REWORK'), true);
    expect(csBannerTitle(cycle: 2, csStatus: 'REWORK'), '재작업 2회차 진행 중');
  });

  test('수선비 환불·보상은 배너 표시', () {
    expect(showCsBanner(cycle: 1, csStatus: 'REPAIR_REFUNDED'), true);
