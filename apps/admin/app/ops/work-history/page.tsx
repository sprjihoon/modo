...
export default function OpsWorkHistoryPage() {
  ...
  const loadWorkHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/ops/work-history?${params.toString()}`);
  ...
  }, [pagination.page, pagination.pageSize, statusFilter, startDate, endDate]);
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold">나의 작업 내역</h1>
        </div>
        <p className="text-muted-foreground">본인이 진행한 수선 작업 내역을 확인하세요</p>
      </div>
