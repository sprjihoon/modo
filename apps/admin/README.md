...
### 직원 관리 (`/dashboard/settings/staff`)
- [x] 직원 목록 조회 (역할별 필터)
- [x] 직원 계정 등록
  - 고객과 이메일 공유 가능 (기존 Auth 계정 재사용)
  - 동일 이메일의 활성 직원이 이미 있으면 차단
- [x] 직원 정보 수정 (이메일·이름·전화번호·권한·비밀번호)
  - 이메일 변경 시 다른 직원 중복만 차단 (고객 이메일 공유 허용)
- [x] 직원 계정 삭제 (soft delete + Auth 계정 삭제)
- [x] 역할 기반 권한 제어 (SUPER_ADMIN / ADMIN / MANAGER / WORKER)

> **관리 분리 원칙**: 직원(`staff` 테이블)과 고객(`users` 테이블)은 별도 테이블로 독립 관리됩니다.
> 같은 이메일을 가진 사람이 고객인 동시에 직원일 수 있습니다.
