...
- [x] 직원 목록 조회 (역할별 필터)
- [x] 직원 계정 등록
  - 신규 이메일만 허용 (직원·고객 계정 완전 분리)
  - 고객 이메일 사용 시 명확한 오류 메시지 반환
- [x] 직원 정보 수정 (이메일·이름·전화번호·권한·비밀번호)
  - 이메일 변경 시 고객 이메일·다른 직원 이메일 중복 차단
- [x] 직원 계정 삭제 (soft delete + Auth 계정 삭제)
- [x] 역할 기반 권한 제어 (SUPER_ADMIN / ADMIN / MANAGER / WORKER)

> **계정 분리 원칙**: 직원 계정과 고객 계정은 완전히 별도의 이메일로 관리됩니다.
> 직원 등록 시 반드시 고객 계정과 무관한 전용 이메일을 사용하세요.
