...
- **미수거 재접수:** `force_rebook: true` + `pickup_date` (YYYY-MM-DD). 기존 우체국 예약을 취소하고 새 송장을 발급. 주문·결제는 유지. 집하완료면 거부. 사용자 JWT면 본인 주문만. `reqNo` 없으면 취소를 건너뛰고 새 접수. UI는 이번 예약이 미수거/부재/신청취소일 때만 버튼을 보여 준다.
