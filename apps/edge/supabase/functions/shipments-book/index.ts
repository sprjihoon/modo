 ...
 *  오늘(KST) 이하이거나 일요일이면 undefined 반환 → 우체국 기본값(내일) 사용
 */
function formatRetVisitYmd(pickupDate: string | null | undefined): string | undefined {
  ...
  // 일요일이면 우체국 기본값 사용
  const dateObj = new Date(`${ymd.slice(0, 4)}-${ymd.slice(4, 6)}-${ymd.slice(6, 8)}T00:00:00Z`);
  if (dateObj.getUTCDay() === 0) {
    console.log(`⚠️ retVisitYmd(${ymd})가 일요일이므로 미전송 → 우체국 기본값(내일) 사용`);
    return undefined;
  }

  console.log(`📅 retVisitYmd 전송: ${ymd}`);
  return ymd;
}
