...
- 스토어 버전/출시 상태: 루트 [`README.md`](../README.md) 「앱스토어 / Play 출시 준비」 (`1.0.6+42`, iOS `1.0.5` 판매 중 · `1.0.6` 42 심사 대기 · Play 38 게시 · 42 검토 대기, MinOS 15). **웹은 `main` 즉시 배포**. 스토어 빌드는 하루 1회 · 맥북에서만 · 오늘은 몇 시간 뒤
- 앱 업데이트 안내: `app_versions` 최신·최소는 iOS/Android 모두 **`1.0.5`** · 어드민 설정 → 앱 버전
- 수선 요청 메모: `orders.customer_memo` · 핀 메모·배송 요청(`notes`)과 별도 · **웹 `modo.io.kr` 라이브** · 작업지시서·어드민 주문상세 공통
