...
배송완료(`DELIVERED`) 주문에 한해 고객이 리뷰를 남긴다. **웹은 라이브**. 앱은 iOS `1.0.7` 판매 중 · 수거정보·쿠폰·수치 키패드·사진 정리·장바구니 5일 만료는 `1.0.8+50` (iOS `1.0.8` 50 심사 중 · Play AAB 50).
...
**지금:** 코드는 `1.0.8+50` (수거정보 입력창 · 쿠폰 셀렉트 · 수치 키패드 · 주문 사진 즉시삭제 · 리뷰 삭제 시 사진 삭제 · 장바구니 5일 만료). iOS는 **`1.0.7` 판매 중** · **`1.0.8` (50) 심사 중 (`WAITING_FOR_REVIEW`)**. Play는 **`1.0.7 (46)` 게시** · 이번 AAB는 **50**. 어드민 `app_versions`는 iOS/Android 모두 **`1.0.5`**. 양쪽 `1.0.7`이 판매 중이라 최신은 **`1.0.7`까지** 올려도 된다. **`1.0.8`은 양쪽 판매 뒤에만.** 명령은 아래 「맥북에서 `1.0.8`」과 `apps/mobile/README.md`에 같다.

**스토어 빌드 규칙:** IPA/AAB는 너무 자주 올리지 않는다. **하루에 한 번만** · **맥북에서만** 한다. Windows에서는 검증만 한다. 웹·어드민은 `main` push 시 Vercel 자동 배포라 이 제한과 무관하다.

**오늘(2026-09-04):** 장바구니 5일 만료. 수거정보 입력창 · 주문 사진 즉시삭제 · 리뷰 삭제 시 사진 삭제. iOS **`1.0.8` (50) `WAITING_FOR_REVIEW`**. Play AAB **`1.0.8+50`** (콘솔 업로드는 아직).

| 항목 | 값 |
|---|---|
| 앱 이름 | 모두의수선 |
| Bundle / Application ID | `com.modurepair.app` |
| 버전 | `apps/mobile/pubspec.yaml` → **`1.0.8+50`**. iOS 심사는 **50**. Play 이번 AAB는 **50**. 다음 Play는 **51** |
| App Store Connect App ID | `6759492888` |
| iOS 스토어 | **판매 중 `1.0.7`**. **`1.0.8` 빌드 50 심사 중** · https://apps.apple.com/kr/app/모두의수선/id6759492888 |
| Play 개발자 계정 | 틸리언 (개인) · Account ID `6272621754721589639` · 본인 확인 완료 |
| Play App ID | `4975768727608817713` |
| Play 상태 | **프로덕션 게시** `1.0.7 (46)` · 이번 AAB **`1.0.8 (50)`** · 대한민국 · https://play.google.com/store/apps/details?id=com.modurepair.app · Alpha opt-in `https://play.google.com/apps/testing/com.modurepair.app` |
...
| Android AAB | 이번 빌드 **`1.0.8+50`**. 백업 `Documents/modo-android-signing/app-release-1.0.8+50.aab`. 다음 업로드는 **51** |
...
| App Store 현재 빌드 | 판매 중 **`1.0.7`**. **`1.0.8` 빌드 50 `WAITING_FOR_REVIEW`** |
...
# Android (Play용 AAB). 이번 빌드 50. 다음 업로드는 51
flutter build appbundle --release --build-name=1.0.8 --build-number=50
# → build/app/outputs/bundle/release/app-release.aab
# 백업: ~/Documents/modo-android-signing/app-release-1.0.8+50.aab

# Android (직접 설치용 APK — Play Protect 경고가 날 수 있음, 신규 서명 키는 흔함)
flutter build apk --release
# → build/app/outputs/flutter-apk/app-release.apk

# iOS (App Store / TestFlight) — 맥북에서만. 제출 빌드는 50
flutter build ipa --release --build-name=1.0.8 --build-number=50 \
...
Windows에서는 IPA/AAB를 만들지 않는다. `pubspec.yaml`은 `1.0.8+50`. iOS는 **`1.0.8` (50) 심사 중**. Play AAB **50**. `1.0.7`이 iOS·Play 판매 중이라 마케팅 버전을 올렸다.
...
iOS **1.0.7** 판매 중 · **1.0.8 (50)** 심사 중. Play는 **`1.0.7 (46)` 게시** · 이번 AAB **`1.0.8 (50)`**. 어드민 **앱 버전** 최신은 **`1.0.7`까지**. `1.0.8`은 양쪽 판매 뒤에만.
...
47. **`1.0.8` 수거정보·쿠폰·수치 키패드·사진 정리·장바구니 5일** — 입력창 흰 화면 수정. 쿠폰 셀렉트. 수치 키패드 닫기. 주문 사진 즉시삭제·60일 정리. 리뷰 삭제시 사진 삭제. 장바구니 5일 만료. iOS **50 `WAITING_FOR_REVIEW`** · Play AAB **50**
...
46. **광고 성과** — 어드민 **분석 → 광고 성과**. 가입 CPA · 주문 CPA · CAC. 광고비는 채널·캠페인·기간으로 직접 입력. 웹 광고 URL UTM이 가입(첫 유입)·주문(결제 직전)에 남음
42. **마케팅 마일스톤** — 아래 **마케팅 마일스톤** 절. P0부터 순서대로 구현. 끝나면 그 표의 상태를 `완료`로 바꾸고 수정 이력에 적는다
...
어드민·웹은 `main` 배포로 적용. 앱 연락처 분리는 **`1.0.6` 판매 중**. 수선 수치 숫자 제한은 **`1.0.7` 판매 중**. 수거정보·쿠폰·수치 키패드·장바구니 5일은 **`1.0.8`** (iOS 50 심사 중 · Play AAB 50).
...
분석이 **보낼 사람·쓸 쿠폰**으로 이어지게 만든다. 끝나면 상태만 `완료`로 바꾸고, 수정 이력 맨 위에 한 줄 추가한다.

이미 쓰는 것(다시 만들지 않음): **마케팅 인사이트**, 고객 행동 분석(퍼널·이탈·시간·디바이스), 배너·팝업·프로모션 코드·공지/푸시·포인트, 고객 목록 마지막 접속.

구현 순서: **휴면 목록 → 장바구니 이탈 → 쿠폰 성적 → 전후 비교 → 골라 보내기 → 배너 클릭 → 재구매·지역·유입 채널**

### P0 — 이번 주 (실행 대상)

| # | 기능 | 하는 일 | 상태 | 화면 |
|---|---|---|---|---|
| 1 | 휴면·재방문 고객 목록 | 30/60일 미접속, 한 번만 사고 조용한 사람. 필터·명수 | 완료 | `/dashboard/analytics/marketing/actions` |
| 2 | 장바구니 이탈 목록 | 담거나 결제 시작했는데 성공 없는 고객 | 완료 | 위 화면 이탈 탭 |
| 3 | 쿠폰 성적표 | 코드별 사용·매출·객단가·신규/재구매. **프로모션 코드 관리와 같은 목록** | 완료 | 위 화면 쿠폰 탭 · `/dashboard/promotions` |

프로모션 코드 관리(`/dashboard/promotions`)에서 **한도**와 **사용**은 다릅니다.

- **한도:** 전체 선착순 횟수(`max_uses`, 비우면 무제한) + 사용자당 횟수(`max_uses_per_user`). 예: 전체 10 · 1인 1 = 선착순 10명, 한 사람 1번.
- **사용 N건:** 그 코드가 **결제된 주문에 붙은 횟수**. 한도를 나중에 1로 바꿔도 이미 결제된 10건은 사용 10으로 남는다.
- 생성·수정은 `/api/admin/promotions`. 웹·앱 견적에서 두 한도를 모두 검사한다.

### P1 — 다음 (효과 확인·발송)

| # | 기능 | 하는 일 | 상태 | 화면 |
|---|---|---|---|---|
| 4 | 캠페인 전후 비교 | 쿠폰/배너 기간 vs 직전 같은 일수. 가입·결제·접속 증감 | 완료 | `/dashboard/analytics/marketing` |
| 5 | 세그먼트 푸시 | 공지 대상을 전체 → 휴면 / 이탈 / 앱만 / 재구매 후보 | 완료 | 공지사항 발송 대상 |
| 6 | 배너·팝업 클릭 | 배너별 클릭 수, 클릭 후 결제. `BANNER_CLICK`은 이미 있음 | 완료 | `/dashboard/analytics/marketing/actions` 배너·팝업 탭 |

### P2 — 이후

| # | 기능 | 하는 일 | 상태 | 화면 |
|---|---|---|---|---|
| 7 | 재구매·주기 | 첫 구매 vs 재구매, 평균 재주문 일수 | 완료 | `/dashboard/analytics/marketing` |
| 8 | 지역 | 수거 주소로 서울/경기 등. 지역 쿠폰 | 완료 | `/dashboard/analytics/marketing` |
| 9 | 접속 경로 캠페인 | 네이버/인스타/구글·UTM별 접속·결제. **2~4주 데이터 쌓인 뒤** | 완료 | `/dashboard/analytics/ads` · 마케팅 인사이트 접속 경로 |

### 보류

배너 노출수. 매체 픽셀(네이버 전환·메타·구글)과 광고비 API 연동은 광고가 커진 뒤에 한다.

---

## 광고 성과

어드민 **분석 → 광고 성과** (`/dashboard/analytics/ads`). 네이버·인스타·구글·유튜브 광고를 같은 표로 비교한다.

| 지표 | 계산 | 귀속 |
|---|---|---|
| 가입 CPA | 광고비 ÷ 가입 수 | 회원 첫 유입 (`users.acq_*`) |
| 주문 CPA | 광고비 ÷ 결제 건수 | 결제 직전 유입 (`orders.acq_*`) |
| CAC | 광고비 ÷ 신규 결제 고객 수 | 그 고객의 첫 유입. 재주문은 분모에 안 넣음 |
| 가입→결제 | 기간 가입자 중 결제 경험 비율 | 첫 유입 캠페인 |

광고 링크 예: `https://modo.io.kr/?utm_source=naver&utm_medium=search&utm_campaign=수선_검색&utm_term=키워드`

광고비는 검색광고센터 등에서 본 금액을 채널·캠페인·기간으로 넣는다. 선택한 조회 기간과 겹치는 날짜만큼 나눈다. SQL: `apps/edge/supabase/migrations/20260902120000_ad_attribution.sql`

검증: `cd apps/admin && npx tsx lib/ad-performance.test.ts` · `cd apps/web && npx tsx lib/acquisition.test.ts`
...
| 2026-09-02 | `1.0.7+46` 재제출 | 45 심사 취소. 수선 수치는 숫자만. Play AAB 백업 `app-release-1.0.7+46.aab` |
...
| 2026-09-01 | 마케팅 마일스톤 | README에 P0~P2 구현 순서 고정. P0부터 구현 |
| 2026-09-01 | 마케팅 인사이트 | 어드민 `/dashboard/analytics/marketing`. 결제 피크 요일·시간, 접속/가입 요일, 요일×시간 히트맵, 푸시 타이밍, 인기 의류·수선, 앱/웹. 고객 목록·상세에 마지막 접속(`customer_events`). 행동 분석 시간 탭에 요일별 접속. 어드민 라이브 |
...
| 2026-09-01 | 스토어 빌드 규칙 | IPA/AAB는 **하루에 한 번만**. 웹은 바로 적용(`modo.io.kr` 라이브) |
| 2026-09-01 | 수선 요청 메모 | 수거정보에 핀 메모·배송 요청과 별도 칸. `orders.customer_memo`. **웹 `modo.io.kr` 라이브**. 앱 `1.0.7+43` |
...
| 2026-08-31 | 부위별 치수 칸 | `허리+힙`은 입력 2개(허리/힙 cm). 부위마다 `input_count`·`input_labels`. 어드민 메뉴에서 수정. 웹·앱 주문 치수에 반영. 스토어 `1.0.6+42` |
