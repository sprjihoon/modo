...
| 3 | 쿠폰 성적표 | 코드별 사용·매출·객단가·신규/재구매. **프로모션 코드 관리와 같은 목록**. 사용은 결제된 주문 기준 | 완료 | 위 화면 쿠폰 탭 · `/dashboard/promotions` |
...
| 날짜 | 항목 | 내용 |
|---|---|---|
| 2026-09-01 | 프로모션 코드 저장·성적 | 어드민 생성이 RLS에 막히던 문제. `/api/admin/promotions`로 저장. 관리 화면과 쿠폰 성적표가 같은 `promotion_codes` 목록. 사용·매출은 결제된 주문 기준. 포인트 전액·0원 주문에도 사용 이력 기록. 첫 배포는 insert 타입 오류로 실패 → 수정 후 재배포 |
