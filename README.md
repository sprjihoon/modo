...
- **왕복 배송비 무료 포함** (`includes_free_shipping`): 공개 코드·CS 전용·미션 쿠폰 발행 시 체크할 수 있다. ON이면 수선 할인과 별도로 **왕복 기본 배송비를 0원**. 도서산간 추가비는 그대로. 이미 있는 배송 프로모와 겹치면 **더 큰 쪽만**. 청구는 `orders-quote`가 계산하므로 구버전 앱도 결제 금액은 맞다. 수거신청 미리보기·쿠폰함 문구는 iOS `1.0.9` 판매 중.
...
- 주문 상세: 어드민 **결제 정보**, 웹·앱 **주문 정보**에 쿠폰(코드)·배송비 할인·포인트 사용을 표시. 없으면 숨김. 웹·어드민은 라이브. **앱 주문 상세 표시는 다음 스토어 빌드** (`order_detail_page.dart` · `order_service.dart`). `1.0.11+53`에는 없음
- 검증: RPC 부분/전액/중복 복구 통과. 기존 실주문 `ORD1788952187223-LTO1N`(BOOKED · 1,000P)은 유지. **이후 취소 건부터** 자동 복구
...
**다음 스토어 빌드(`1.0.12` 예정)에 넣을 것:** 앱 주문 상세 **주문 정보**에 쿠폰(코드)·배송비 할인·포인트 사용 표시. 송화인 부재 등 **미수거 재접수** 버튼. 지금 따로 IPA/AAB 올리지 않는다. 웹·어드민은 `main` 배포. 재접수 API는 Edge라 스토어 앱에서도 관리자·웹으로 처리할 수 있다.
...
우체국이 **송화인 부재·미수거·신청취소**로 수거하지 못하면 주문·결제는 유지한 채 기존 송장을 취소하고 새 수거일로 다시 접수한다.

**버튼은 이번 예약이 실패했을 때만.** 신규 수거예약(`수거예약 완료`만 있는 건)과 재접수가 끝난 건에는 숨긴다. 이전 미수거 이력은 추적에 남긴다.

| 경로 | 동작 |
|---|---|
| Edge | `shipments-book` + `force_rebook: true` — 우체국 취소 후 새 송장. 집하완료면 거부. `reqNo` 없으면 취소 건너뛰고 새 접수 |
| 어드민 | 주문 상세 **미수거 재접수**. `POST /api/orders/[id]/book-pickup` `{ forceRebook: true, pickupDate }`. **라이브** |
| 고객 웹 | 주문 상세·배송추적. `POST /api/orders/[id]/rebook-pickup`. **라이브** |
| 앱 | 주문 상세 **수거 다시 예약하기**. 같은 Edge. **다음 스토어 빌드(`1.0.12`)** |

주문 취소·환불과 다르다. `shipments-cancel`은 주문까지 취소한다.

추적 스크래핑이 예약번호(`reqNo`)를 지워도 `delivery_info`에 남겨 둔다. ERR-211 신청번호 누락을 고객번호 오류로 바꾸지 않는다.
...
| 날짜 | 항목 | 내용 |
|---|---|---|
| 2026-09-11 | 재접수 버튼 조건 | 신규 예약에는 숨김. 이번 예약 이후 송화인부재/미수거/신청취소일 때만 표시. 재접수 후 숨김. 이전 실패 이력은 추적에 유지. 어드민·웹 `main` 라이브 (`49fb308`·`e1cc3b1`). 앱 버튼은 `1.0.12` |
