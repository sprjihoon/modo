...
배송완료(`DELIVERED`) 주문에 한해 고객이 리뷰를 남긴다. **웹은 라이브**. 앱은 iOS `1.0.7` 판매 중 · 수거정보·쿠폰·수치 키패드·사진 정리·장바구니 5일 만료는 `1.0.8+50` (iOS 49 심사 취소 후 50 제출 · Play AAB 50).
...
**지금:** 코드는 `1.0.8+50` (수거정보 입력창 · 쿠폰 셀렉트 · 수치 키패드 · 주문 사진 즉시삭제 · 리뷰 삭제 시 사진 삭제 · 장바구니 5일 만료). iOS는 **`1.0.7` 판매 중** · **`1.0.8` 49 심사 취소 후 50 제출**. Play는 **`1.0.7 (46)` 게시** · 이번 AAB는 **50**. 어드민 `app_versions`는 iOS/Android 모두 **`1.0.5`**. 양쪽 `1.0.7`이 판매 중이라 최신은 **`1.0.7`까지** 올려도 된다. **`1.0.8`은 양쪽 판매 뒤에만.** 명령은 아래 「맥북에서 `1.0.8`」과 `apps/mobile/README.md`에 같다.

**스토어 빌드 규칙:** IPA/AAB는 너무 자주 올리지 않는다. **하루에 한 번만** · **맥북에서만** 한다. Windows에서는 검증만 한다. 웹·어드민은 `main` push 시 Vercel 자동 배포라 이 제한과 무관하다.

**오늘(2026-09-04):** 장바구니 5일 만료. 수거정보 입력창 · 주문 사진 즉시삭제 · 리뷰 삭제 시 사진 삭제. iOS **49 심사 취소 후 `1.0.8 (50)` 제출**. Play AAB **`1.0.8+50`**.
...
| iOS 스토어 | **판매 중 `1.0.7`**. **`1.0.8` 빌드 50 제출** · https://apps.apple.com/kr/app/모두의수선/id6759492888 |
...
| App Store 현재 빌드 | 판매 중 **`1.0.7`**. **`1.0.8` 빌드 50 제출** |
...
Windows에서는 IPA/AAB를 만들지 않는다. `pubspec.yaml`은 `1.0.8+50`. iOS는 **49 심사 취소 후 50 제출**. Play AAB **50**. `1.0.7`이 iOS·Play 판매 중이라 마케팅 버전을 올렸다.
...
iOS **1.0.7** 판매 중 · **1.0.8 (50)** 제출. Play는 **`1.0.7 (46)` 게시** · 이번 AAB **`1.0.8 (50)`**. 어드민 **앱 버전** 최신은 **`1.0.7`까지**. `1.0.8`은 양쪽 판매 뒤에만.
...
47. **`1.0.8` 수거정보·쿠폰·수치 키패드·사진 정리·장바구니 5일** — 입력창 흰 화면 수정. 쿠폰 셀렉트. 수치 키패드 닫기. 주문 사진 즉시삭제·60일 정리. 리뷰 삭제시 사진 삭제. 장바구니 5일 만료. iOS 49 취소 후 **50 제출** · Play AAB **50**
...
| 날짜 | 항목 | 내용 |
|---|---|---|
| 2026-09-04 | `1.0.8+50` | 장바구니 5일 만료. 수거정보 흰 화면·쿠폰 셀렉트·수치 키패드. 주문 사진 즉시삭제·60일 정리. 리뷰 삭제시 사진 삭제. iOS 49 취소 후 50 제출. Play AAB 50 |
